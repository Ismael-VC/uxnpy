...
/*
Copyright (u) 2022-2025 Devine Lu Linvega, Andrew Alderwick, Andrew Richards,
...
uxn_eval(Uxn *u, Uint16 pc)
{
    ...
    u->pc = pc;
    if(!u->pc || u->dev[0x0f]) return 0;
    for(step = STEP_MAX; step; step--) {
        switch(u->ram[u->pc++]) {
        /* BRK */ case 0x00: return 1;
        /* JCI */ case 0x20: if(DEC(wst)) { JMI break; } u->pc += 2; break;
        /* JMI */ case 0x40: JMI break;
        /* JSI */ case 0x60: c = u->pc + 2; INC(rst) = c >> 8; INC(rst) = c; JMI break;
        /* LI2 */ case 0xa0: INC(wst) = u->ram[u->pc++]; /* fall-through */
        /* LIT */ case 0x80: INC(wst) = u->ram[u->pc++]; break;
        /* L2r */ case 0xe0: INC(rst) = u->ram[u->pc++]; /* fall-through */
        /* LIr */ case 0xc0: INC(rst) = u->ram[u->pc++]; break;
        ...
        /* JSR */ OPC(0x0e,POx(a),RP1(u->pc >> 8) RP1(u->pc) JMP(a))
        /* STH */ OPC(0x0f,GET(x),RP1(x[0]) if(_2) RP1(x[1]))
        /* LDZ */ OPC(0x10,PO1(a),PEK(a, x, 0xff))
        /* STZ */ OPC(0x11,PO1(a) GET(y),POK(a, y, 0xff))
        /* LDR */ OPC(0x12,PO1(a),PEK(u->pc + (Sint8)a, x, 0xffff))
        /* STR */ OPC(0x13,PO1(a) GET(y),POK(u->pc + (Sint8)a, y, 0xffff))
